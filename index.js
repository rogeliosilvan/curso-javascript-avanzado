...
// console.log(inventory.reduce((last, next) => ({value: last.value + next.value})).value)


const teamName = "tooling"
const employees = [{name: "Jennie", role: "senior"},
                {name: "Ronald", role: "junior"},
                {name: "Martin", role: "senior"},
                {name: "Anneli", role: "junior"},
                {name: "Sergio", role: "senior"}
              ]

const format = (teamName, people) => `There are ${people.length} people on the ${teamName} team. 
Their names are ${getNames(people)}. 
${getNumberOfSeniors(people)} of them have a senior role.`

const getNames = people => people.map(person => person.name).join(', ')
const getNumberOfSeniors = people => people.filter(person => person.role === 'senior').length

// console.log(format(teamName, employees))
...
let { name, firstName, age, dad: { name: dadName }, mother: { age: motherAge } } = getFamily()

console.log(name, firstName, age, dadName, motherAge)
...
let property = Symbol('name')

let objectWithPrivateProperty = {
  age: 25,
  [property]: 'Sergio'
}

console.log(objectWithPrivateProperty, objectWithPrivateProperty.name, objectWithPrivateProperty[property])
